...
#ifndef XRPL_PROTOCOL_BOOKCHANGES_H_INCLUDED
#define XRPL_PROTOCOL_BOOKCHANGES_H_INCLUDED
...
/** Compute book changes from a ledger.
 *
 * This function analyzes all transactions in the provided ledger and computes
 * the aggregate book changes (volume, high/low/open/close rates) for each
 * trading pair that had activity.
 *
 * @tparam L Ledger type (must provide txs, header())
 * @param lpAccepted The accepted ledger to analyze
 * @return Json::Value containing book changes data
 */
template <class L>
Json::Value
